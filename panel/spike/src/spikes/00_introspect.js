...
function listProps(obj, label, log) {
    if (obj === null || obj === undefined) {
        log.warn(`${label}: <${obj === null ? "null" : "undefined"}>`);
        return;
    }
    const filterBoring = (k) => k !== "length" &&
        k !== "name" &&
        k !== "arguments" &&
        k !== "caller" &&
        k !== "prototype" &&
    ...
    let ownKeys = [];
    try {
        ownKeys = Object.getOwnPropertyNames(obj).filter(filterBoring);
    }
    catch (e) {
        log.warn(`${label}: getOwnPropertyNames threw: ${e.message}`);
    }
    const protoChain = [];
    try {
        ...
        log.warn(`${label}: prototype walk threw: ${e.message}`);
    }
        ...
        try {
            ...
            try {
                ...
                if (markerColor && typeof markerColor === "object") {
                    const entries = Object.entries(markerColor);
                ...
                if (trackItemType && typeof trackItemType === "object") {
                    ...
                    for (const [k, val] of Object.entries(trackItemType)) {
                        log.info(`  ${k} = ${JSON.stringify(val)}`);
                    }
                }
            }
            catch (e) {
                log.warn(`Constants enum walk threw: ${e.message}`);
            }
            ...
            try {
                ...
                if (typeof guidProp === "function") {
                    const g = await Promise.resolve(guidProp.call(project));
                ...
                log.warn(`project.guid read threw: ${e.message}`);
            }
            // 3b. Active sequence — already known proto, also read guid/name
            const seq = (await projectAny.getActiveSequence());
            if (seq) {
                try {
                    const g = seq.guid;
                    if (typeof g === "function") {
                        log.ok(`sequence.guid() → ${JSON.stringify(await Promise.resolve(g.call(seq)))}`);
                    }
                    else {
                    ...
                    log.warn(`sequence.guid threw: ${e.message}`);
                }
                // VideoTrack instance
                try {
                    const vtCount = seq.getVideoTrackCount;
                    const count = vtCount ? await Promise.resolve(vtCount.call(seq)) : -1;
                    log.info(`videoTrackCount = ${count}`);
                    if (typeof count === "number" && count > 0) {
                        const getVt = seq.getVideoTrack;
                        const vt = await Promise.resolve(getVt.call(seq, 0));
                        listProps(vt, "videoTrack[0] (instance)", log);
                    }
                }
                catch (e) {
                    log.warn(`videoTrack probe threw: ${e.message}`);
                }
            }
            // 3c. Root item → first child (likely ClipProjectItem)
            const rootItem = (await Promise.resolve(projectAny.getRootItem()));
            if (rootItem) {
                try {
                    const items = await Promise.resolve(rootItem.getItems.call(rootItem));
                    const arr = items;
                    ...
                    for (let i = 0; i < Math.min(arr.length, 3); i++) {
                        ...
                        for (const m of candidates) {
                            if (m in it)
                                log.info(`  [${i}].${m} :: ${describeValue(it[m])}`);
                        }
                        ...
                        for (const m of ["getId", "getName"]) {
                            ...
                            if (typeof fn === "function") {
                                try {
                                    const r = await Promise.resolve(fn.call(it));
                    ...
                    log.warn(`rootItem.getItems() threw: ${e.message}`);
                }
            }
